...
PYTHONPATH=..:$PYTHONPATH OMP_NUM_THREADS=4 torchrun  \
    --nproc_per_node=8  \
    gen_model/gen_moe_default.py --policy moe \
        --layers 2 \
        --hidden 1024\
        --heads 32\
        --dp_size 2 \
...
PYTHONPATH=..:$PYTHONPATH OMP_NUM_THREADS=4 torchrun  \
    --nproc_per_node=1  \
    gen_model/gen_moe_default.py --policy dp \
        --layers 2 \
        --hidden 1024\
        --heads 32\
        --dp_size 1 \
...
from Verdict.gen_model.model.moe import Transformer, ModelArgs
...
def parse_arguments():
    ...
    parser.add_argument('--act_expt', type=int, help='sequence length')
    parser.add_argument('--rout_expt', type=int, help='sequence length')
...
def render_model_args(args):
    ...
    model_args.n_activated_experts = args.act_expt or model_args.n_activated_experts
    model_args.n_routed_experts = args.rout_expt or model_args.n_routed_experts
    return model_args
...
def create_path(args, model_args: ModelArgs):
    ...
    strategy = "default"
    fname = f"moe_{strategy}_dp{dp}_pp{pp}_tp{tp}_nm{nm}_gbs{gbs}_ly{layers}_h{h}_hi{hi}_sq{sql}_a{model_args.n_activated_experts}_r{model_args.n_routed_experts}"
