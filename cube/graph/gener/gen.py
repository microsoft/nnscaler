...
class IRAdapterGener:
    ...
    def gen_activation(graph: IRSegment, allow_recompute: bool = True, cost_fn: Optional[Callable] = None) -> IRSegment:
        ...
        # an operator may require more than one adapters for its input tensors,
        # when the code is compiled at multiple servers, we need to 
        # reorder ftensor to make consistent ordering when generating communications
        ftensors = sorted(ftensors, key=lambda t: t.tid)
