...
def memory_summary() -> int:
    torch.cuda.synchronize()
    # memory measurement
    allocated_mem = torch.cuda.max_memory_allocated()
    amem = round(allocated_mem / 1024 / 1024 / 1024, 2)
    preserved_mem = torch.cuda.max_memory_reserved()
    pmem = round(preserved_mem / 1024 / 1024 / 1024, 2)
    print_each_rank(
        f'memory max allocated {amem} GB | max preserved {pmem} GB',
        logger=_logger
    )
    return amem
