...
Note that in the command above, we enable searching for pipeline parallelism by passing `--explore_pipeline` and set the possible pipeline stage boundaries by `--pipeline_pivots LlamaDecoderLayer`.
