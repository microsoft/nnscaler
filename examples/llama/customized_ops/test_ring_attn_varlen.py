...
from ring_attention import wrap_ring_attn_varlen_func
...
class TestModule(torch.nn.Module):
    ...
    def forward(self, q, k, v, cu_seqlens_q, cu_seqlens_k):
        out = wrap_ring_attn_varlen_func(q, k, v, cu_seqlens_q, cu_seqlens_k)
        return out
...
def policy(graph: IRGraph, resource: ComputeConfig) -> IRGraph:
    ...
    for idx, node in enumerate(graph.select(ntype=IRFwOperation)):
        if not partitioned and node.signature == 'ring_attention.ring_attn_varlen.wrap_ring_attn_varlen_func':
...
if __name__ == "__main__":
    ...
    cu_seqlens = [0, 120, 1248, 4232, 8192]
    ...
    single_out = wrap_ring_attn_varlen_func(q, k, v, cu_seqlens_tensor, cu_seqlens_tensor)
    ...
    parallel_model = parallelize(
                        model,
                        dummy_forward_args={"q": qq, "k": kk, "v": vv, 'cu_seqlens_q': cu_seqlens_tensor, 'cu_seqlens_k': cu_seqlens_tensor},
                        pas_policy=policy,
    ...
    para_out = parallel_model(qq, kk, vv, cu_seqlens_tensor, cu_seqlens_tensor)
