...
from genmodel.model.llama3_adpt import Transformer, ModelArgs, MOCK
...
heads = args.heads
seqlen = args.seqlen
nm = gbs//dp//mbs if args.policy in ["hybrid", "pp", "flex"] else 1
prefix = 'adptMegE'
if args.policy=='flex':
    prefix = 'flex'
elif args.policy=='tp':
    prefix = 'randtp'
fname = f"llama3{prefix}_mgener_dp{args.dp_size}_pp{args.pp_size}_tp{args.tp_size}_nm{nm}_gbs{gbs}_ly{layers}_h{heads}_hi{hidden}_sq{seqlen}"
